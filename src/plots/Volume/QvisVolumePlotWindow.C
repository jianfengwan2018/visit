...
QvisVolumePlotWindow::UpdateWindow(bool doAll)
{
    ...
    for(int i = 0; i < volumeAtts->NumAttributes(); ++i)
    {
        ...
        case VolumeAttributes::ID_legendFlag:
            ...
            customLegendTitleToggle->setEnabled(volumeAtts->GetLegendFlag());
            customLegendTitle->setEnabled(volumeAtts->GetLegendFlag() &&
                                    volumeAtts->GetCustomLegendTitleEnabled());
            legendToggle->blockSignals(false);
            break;
        case VolumeAttributes::ID_customLegendTitleEnabled:
            customLegendTitleToggle->blockSignals(true);
            customLegendTitleToggle->setChecked(volumeAtts->GetCustomLegendTitleEnabled());
            customLegendTitle->setEnabled(volumeAtts->GetLegendFlag() &&
                                    volumeAtts->GetCustomLegendTitleEnabled());
            customLegendTitleToggle->blockSignals(false);
            break;
        case VolumeAttributes::ID_customLegendTitle:
            customLegendTitle->blockSignals(true);
            customLegendTitle->setText(volumeAtts->GetCustomLegendTitle().c_str());
...
// ****************************************************************************
// Method: QvisVolumePlotWindow::customLegendTitleToggled
//
// Purpose:
//   This is a Qt slot function that is called when the custom legend toggle
//   is clicked.
//
...
void
QvisVolumePlotWindow::customLegendTitleToggled(bool val)
{
    volumeAtts->SetCustomLegendTitleEnabled(val);
    Apply();
}

// ****************************************************************************
// Method: QvisVolumePlotWindow::customLegendTitleProcessText
//
// Purpose:
...
// ****************************************************************************
void
QvisVolumePlotWindow::customLegendTitleProcessText()
{
    // Only do it if it changed.
    if(customLegendTitle->text().toStdString() != volumeAtts->GetCustomLegendTitle())
    {
        volumeAtts->SetCustomLegendTitle(customLegendTitle->text().toStdString());
        Apply();
    }
}
